...
function BuildAssets( i_path_assetsToBuild )
		...
		local sourceLicenses = GetFilesInDirectory( LicenseDir )
			for i, sourceLicense in ipairs( sourceLicenses ) do
				...
				if shouldTargetBeBuilt then
					...
					if result then
							-- Display a message
							print( "Installed Lisenses" )
					else
						wereThereErrors = true
						OutputErrorMessage( "Lisenses couldn't be copied \": " .. errorMessage )
					end
				end 
			end 
		end
	...
	do
		...
		local setting_authored = OutputDir .. "settings.ini"
		...
		do
			...
			if doesTargetExist then
				...
				-- If the source has been modified more recently than the target
				-- then the target should be re-built.
				local lastWriteTime_source = GetLastWriteTime( setting_authored )
				local lastWriteTime_target = GetLastWriteTime( targetPath )
				shouldTargetBeBuilt = lastWriteTime_source > lastWriteTime_target
		...
		if shouldTargetBeBuilt then
			local result, errorMessage = CopyFile(setting_authored , targetPath)
			if result then
					-- Display a message
					print( "Installed settings.ini" )
				else
					wereThereErrors = true
					OutputErrorMessage( "The settings.ini  couldn't be copied \": " .. errorMessage )
				end
		end 
	end
	end
