...
namespace
{
		...
		const std::vector<sMaterialInfo>& i_materialInfo )
	{
		...
		for (int i = 0; i < i_indexArray.size(); i++)
		{
			...
			//std::swap(i_indexArray[i + 1], i_indexArray[i + 2]);
		}
...
//			fout << std::fixed;
				...
				"{\n";
			{
				//Start of index data
				fout << "IndexData = { \n";
				for each ( size_t var in i_indexArray)
				{
					fout << var << " , ";
				}
				fout << "}, \n";
				...
				for each(sVertexInfo var in i_vertexArray)
				{
					fout << "{\n";
					fout << "position = { " << var.vertex.x << " , " << var.vertex.y << " , " << var.vertex.z << " , } , ";

					fout << "color = { " << var.vertex.r << " , " << var.vertex.g << " , " << var.vertex.b << " , " <<var .vertex.a << " , } , ";

					fout << "uv = { " << var.vertex.u << " , " << var.vertex.v << " , } ,  ";
